  ...
  const downloadImgShare = async (downloadType: string) => {
    ...
    try {
      ...
      if (codeAreaRef.current) {
        ...
        switch (downloadType) {
          case "SVG":
            dataUrl = await toSvg(codeAreaRef.current, options);
            link.download = "codeArea.svg";
            link.href = dataUrl;
            break;

          case "JPG":
            dataUrl = await toJpeg(codeAreaRef.current, options);
            link.download = "codeArea.jpg";
            link.href = dataUrl;
            break;

          case "PNG":
            dataUrl = await toPng(codeAreaRef.current, options);
      ...
      // Restaura o border-radius e o botão de compartilhamento
      if (containerDiv) {
